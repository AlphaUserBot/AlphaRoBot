...
class InteractiveTelegramClient(TelegramClient):
    def __init__(self, session_user_id, api_id, api_hash,
        ...
        self.found_media = {}
        print('@AlphaUserBot String Alıcıya Xoş Gəldiniz')
        print('[i] Telegramın Serverlərinə bağlanılır...')
        try:
            loop.run_until_complete(self.connect())
        except IOError:
            print('[!] Bağlanarkən bir xəta yarandı. Yenidən yoxlanılır...')
            loop.run_until_complete(self.connect())

        if not loop.run_until_complete(self.is_user_authorized()):
            if telefon == None:
               user_phone = input('[?] Telefon nömrəniz (Məsələn: +994xxxxxxxx): ')
            else:
               user_phone = telefon
            try:
                ...
                print("[!] Yanlış nömrə yazdınız. Məsələn : +994xxxxxxxxxx")
                exit(1)
            except ValueError:
               print("[!] Yanlış bir nömrə yazdınız. Məsələn: +994xxxxxxxxxx")
               exit(1)

            while self_user is None:
                code = input('[?] Telegramdan Gələn Beş (5) Xanalı Kodu Yazın: ')
                try:
                    self_user =\
                        loop.run_until_complete(self.sign_in(code=code))
                except PhoneCodeInvalidError:
                    print("[!] Kodu səhv yazdınız. Xahiş edirəm yenidən yoxlayın. [Çoxlu yoxlamaq hesabınızın ban olunmasına səbəb ola bilər]")
